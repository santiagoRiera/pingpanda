...
export const POST = async (req: NextRequest) => {
  try {
    ...
    //if there is an error, zod will throw an error
    ...
    try {
      //enviamos el mensaje (ya formateado para ds) al usuario en ds
      await discord.sendEmbed(dmChannel.id, eventData)

      //actualizamos el evento que acababamos de crear
      await db.event.update({
        where: { id: event.id },
        data: { deliveryStatus: "DELIVERED" },
      })

      //upsert = updaate or insert
      await db.quota.upsert({
        where: { userId: user.id, month: currentMonth, year: currentYear },
        update: { count: { increment: 1 } },
        //si no existe la cuota de este mes y año, la creamos
        create: {
          userId: user.id,
          month: currentMonth,
          year: currentYear,
