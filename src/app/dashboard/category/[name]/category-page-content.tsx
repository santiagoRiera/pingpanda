...
export const CategoryPageContent = ({
  ...
  const [pagination, setPagination] = useState({
    pageIndex: page - 1,
    pageSize: limit, //how many elements are per page
  })


  const { data, isFetching } = useQuery({
    //if the user change the page or the size of the table or the activetab, we automatically refetch the data
    queryKey: [
      "events",
      category.name,
    ...
    refetchOnWindowFocus: false, //so not every time we switch back and fort in tabs react refetches the data
    enabled: pollingData.hasEvents, //toda la query se habilita solo si tenemos eventos
  })


  //first, we get the columns of the table. We have multiple tables [] of type Event
  const columns: ColumnDef<Event>[] = useMemo(
    () => [
      {
        accessorKey: "category",
        header: "Category", //text that is shwon
      ...
      },
      {
        ...
        cell: ({ row }) => {
          //we get the value of the row referencing the key
  ...
  const table = useReactTable({
    ...
    onSortingChange: setSorting, //user can sort the table
    getSortedRowModel: getSortedRowModel(),
    onColumnFiltersChange: setColumnFilters,
    getFilteredRowModel: getFilteredRowModel(),
    getPaginationRowModel: getPaginationRowModel(),
    manualPagination: true,
    pageCount: Math.ceil((data?.eventsCount || 0) / pagination.pageSize), //redondeamos hacia arriba el num de paginas
 ...
 const router = useRouter()
  ...
  //useMemo to avoid recalculating the sums every time the component re-renders unless we especify
  const numericFieldSums = useMemo(() => {
    if (!data?.events || data.events.length === 0) return {}

    //sum up all the unique fields for the time selected
    const sums: Record<
      string, //key
      {//args
        total: number
    ...
    data.events.forEach((event) => {
      ...
      Object.entries(event.fields as object).forEach(([field, value]) => {
        if (typeof value === "number") {
          ...
          sums[field].total += value //todo el total
  ...
  //esto debe ir abajo o causa problemas con los react hooks condicionales
                  ...
                  <TableRow key={row.id}>
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id}>
                        {flexRender(
                          cell.column.columnDef.cell, //lo que va dentro de la cell
                          cell.getContext()
                        )}
                      </TableCell>
                    ))}
